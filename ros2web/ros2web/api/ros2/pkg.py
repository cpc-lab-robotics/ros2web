...
def _get_package_manifest(package_name) -> Optional[PackageManifest]:
    try:
        package_share_dir = get_package_share_directory(package_name)
    except PackageNotFoundError:
        return None
    
    return _get_package_manifest_by_path(package_share_dir)
...
class ROS2PackageAPI:
    ...
    async def manifest(self, package_name) -> Optional[PackageManifest]:
        descriptor = await self.__loop.run_in_executor(None, _get_package_manifest, package_name)
        return descriptor
